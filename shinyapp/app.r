  ...
  ui <- fluidPage(#titlePanel("doseR | Alpha release"),
    ...
    navbarPage(
      ...
      tabPanel(
        ...
        img(src='doseR_model.png', height="100%", width="100%", align = "center"), 
        p(
          "References: ", 
          br(),
  ...
  server <- function(input, output) {
    ...
    data <- reactive({
      if (DRmodel() == "Time-dependent") {
        data.frame(f_simulate(targets, nn, input$pinf, input$pcont, input$kk, tt, input$cc))
      }
    })
    ...
    plot_diseased = ggplot(data(), aes(x = tt, y = diseased, group = sim)) +
      geom_line(#aes(color = as.factor(D_target)), 
                color = "#9e9ac8", 
                size = 1) +
      geom_point(#aes(fill = as.factor(D_target)), 
                 fill = "#54278f",
